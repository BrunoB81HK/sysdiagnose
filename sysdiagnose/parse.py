...
def parse_main(args: argparse.Namespace) -> int:
...
def parse(parser: str, case_id: str) -> int:
    ...
    spec = importlib.util.spec_from_file_location(
        parser[:-3], config.parsers_folder + parser + ".py"
    )
    ...
    if isinstance(case[module.parser_input], str):
        command = (
            "module." + module.parser_call + "('" + case[module.parser_input] + "')"
        )
    else:
        command = (
            "module." + module.parser_call + "(" + str(case[module.parser_input]) + ")"
        )
